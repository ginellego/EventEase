...
public class RevisitDeclinedRequestsSystemTest {
    ...
    @Test
    public void testRevisitDeclinedRequest() {
        ...
        //should match text to "Service Type 2"
        Espresso.onView(withId(R.id.service_type)).check(matches(withText("Service Type 2")));
        pressBack();
        Espresso.onView(allOf(withId(R.id.r_requestsRecView), isDisplayed())).perform(RecyclerViewActions.actionOnItemAtPosition(0, longClick()));
        Espresso.onView(withText("What would you like to do with this request?"))
                .inRoot(isDialog())
                .check(matches(isDisplayed()));
        Espresso.onView(withText("ACCEPT"))
                .inRoot(isDialog())
                .perform(click());
        pressBack();
